...
export interface Post {
  ...
  video?: string // Magnet link for WebTorrent video
  timestamp: number
  likes: number
  replies?: number
  comments?: Post[]
...
export class PigeonSocialService {
  ...
  private async initializeWebTorrent() {
    ...
    if (this.webtorrentInitializing) {
      while (this.webtorrentInitializing) {
        await new Promise(resolve => setTimeout(resolve, 100))
      }
      return
    }

    this.webtorrentInitializing = true
    ...
    try {
      ...
      this.webtorrentInitializing = false
    }
  }

  private async loadWebTorrent(): Promise<any> {
    // Check if WebTorrent is already available globally
    ...
    throw new Error('WebTorrent not available after waiting')
  }
  ...
  async createPost(content: string, image?: string, videoFile?: File): Promise<Post> {
    ...
    const post: Post = {
      ...
      content,
      image,
      // video: videoMagnetURI, // Temporarily disabled
    ...
    console.log('📝 [createPost] Created post object:', {
      ...
      hasVideo: !!post.video,
      videoMagnet: post.video?.substring(0, 50) + '...' || 'none'
    })
  ...
  private async createVideoTorrent(videoFile: File): Promise<string> {
    console.log('🎬 [createVideoTorrent] Starting with file:', {
      name: videoFile.name,
      size: videoFile.size,
      type: videoFile.type
    })
    
    // WebTorrent should already be initialized, but double-check
    if (!this.webtorrentClient) {
      console.log('⏳ [createVideoTorrent] WebTorrent not ready, initializing...')
      await this.initializeWebTorrent()

      // If still not initialized after waiting, throw error
      if (!this.webtorrentClient) {
        console.log('❌ [createVideoTorrent] WebTorrent failed to initialize')
        throw new Error('WebTorrent failed to initialize')
      }
    }

    console.log('✅ [createVideoTorrent] WebTorrent client ready')

    return new Promise((resolve, reject) => {
      console.log('🌱 [createVideoTorrent] Starting to seed - EXACTLY like wt.js')
      
      // EXACTLY like wt.js - just seed and use callback immediately
      this.webtorrentClient!.seed(videoFile, (torrent: any) => {
        console.log('🎉 [createVideoTorrent] Torrent created - EXACTLY like wt.js!')
        console.log('📹 [createVideoTorrent] magnetURI:', torrent.magnetURI.substring(0, 50) + '...')
        console.log('� [createVideoTorrent] infoHash:', torrent.infoHash)
        
        // Store like wt.js: activeTorrents.set(infoHash, torrentInfo)
        this.activeTorrents.set(torrent.infoHash, {
          magnetURI: torrent.magnetURI,
          torrent: torrent, // Store the actual torrent object like wt.js
          createdAt: new Date()
        })
        console.log('📹 [createVideoTorrent] Stored in activeTorrents:', torrent.infoHash)
      ...
      // Simple timeout
      setTimeout(() => {
        console.error('❌ [createVideoTorrent] Timeout')
        reject(new Error('Timeout'))
      }, 30000)
    })
  }

  // Get an active torrent by infoHash - like wt.js activeTorrents.get()
  getTorrentByInfoHash(infoHash: string): any {
    return this.activeTorrents.get(infoHash)
  }

  // Call this when the feed view changes (e.g., from UI on scroll/page change)
  async updateVisiblePosts(postIds: string[]): Promise<void> {
    // Ensure WebTorrent is initialized
    await this.initializeWebTorrent()
    
    const newVisible = new Set(postIds)
    const toAdd = postIds.filter(id => !this.visiblePostIds.has(id))
    const toRemove = Array.from(this.visiblePostIds).filter(id => !newVisible.has(id))

    // Add torrents for new visible posts (with buffer)
    for (const postId of toAdd.slice(0, this.bufferSize)) {
      await this.addTorrentForPost(postId)
    }

    // Remove torrents for posts out of view/buffer
    for (const postId of toRemove) {
      this.removeTorrentForPost(postId)
    }

    this.visiblePostIds = newVisible
  }

  private async addTorrentForPost(postId: string): Promise<void> {
    console.log('🎬 addTorrentForPost called for:', postId)
    
    // Handle shared posts - extract the original post ID
    let lookupPostId = postId
    if (postId.startsWith('shared_')) {
      const parts = postId.split('_')
      if (parts.length >= 3) {
        // Remove 'shared' and timestamp, keep the middle part(s)
        lookupPostId = parts.slice(1, -1).join('_')
        console.log('🔍 Extracted original post ID from shared post:', lookupPostId)
      }
    }
    
    if (this.activeTorrents.has(postId)) {
      console.log('✅ Torrent already exists for post:', postId)
      return
    }

    const post = await this.getPost(postId)
    console.log('📝 Retrieved post for torrent:', post)
    
    if (!post?.video) {
      console.log('❌ No video found in post:', postId)
      return
    }

    console.log('🔗 Video magnet URI:', post.video)
    ...
    if (!this.webtorrentClient) {
      console.log('❌ WebTorrent client failed to initialize')
      return
    }
    ...
    const existingTorrent = this.webtorrentClient.get(post.video)
    if (existingTorrent) {
      this.activeTorrents.set(postId, existingTorrent)
      console.log('📹 Using existing torrent for post:', postId)
      return
    }

    try {
      console.log('🔽 Adding new torrent for post:', postId)
      
      const torrent = this.webtorrentClient.add(post.video, {
      ...
      this.activeTorrents.set(postId, torrent)
      console.log('📹 Leeching/seeding video for post:', postId)
      
      // Set up event handlers for better debugging
      torrent.on('ready', () => {
        console.log('✅ Torrent ready for post:', postId, 'files:', torrent.files?.length || 0)
      })
      
      torrent.on('metadata', () => {
        console.log('📋 Metadata received for post:', postId, 'files:', torrent.files?.length || 0)
      })
      
      torrent.on('download', () => {
        console.log('📥 Download progress for post:', postId, 'progress:', Math.round(torrent.progress * 100) + '%')
      })
      
      torrent.on('error', (err: Error) => {
        console.error('❌ Torrent error for post:', postId, err)
        this.activeTorrents.delete(postId)
      })
      
    } catch (error) {
      console.error('❌ Failed to add torrent for post:', postId, error)
    }
  }

  private removeTorrentForPost(postId: string): void {
    const torrent = this.activeTorrents.get(postId)
    if (torrent) {
      // Don't destroy the torrent if it's being used by other posts
      const magnetURI = torrent.magnetURI
      const otherPostsUsingTorrent = Array.from(this.activeTorrents.entries()).filter(
        ([id, t]) => id !== postId && t.magnetURI === magnetURI
      )
      
      this.activeTorrents.delete(postId)
      
      if (otherPostsUsingTorrent.length === 0) {
        // Only destroy if no other posts are using this torrent
        torrent.destroy()
        console.log('📹 Removed and destroyed torrent for post:', postId)
      } else {
        console.log('📹 Removed reference to torrent for post (still used by others):', postId)
      }
    }
  }

  // Public method to get active torrent for a post
  getActiveTorrent(postId: string): any {
    const torrent = this.activeTorrents.get(postId)
    if (torrent) {
      return torrent
    }
  ...
  // Public method to get WebTorrent client
  async getWebTorrentClient(): Promise<any> {
    if (!this.webtorrentClient) {
      await this.initializeWebTorrent()
    }
        ...
        for (const postInfo of monthlyPosts.slice(0, limit)) {
          try {
            ...
            if (fullPost) {
              ...
              // Save the post locally for future access
              await this.saveFollowedPost(fullPost)
            } else {
  ...
  async getPost(postId: string): Promise<Post | null> {
    ...
    // First try localStorage (where our own posts are stored)
    const localKey = `pigeon:posts.${lookupPostId}`
    const localPost = localStorage.getItem(localKey)
    if (localPost) {
      try {
        const post = JSON.parse(localPost)
        console.log('✅ Found post in localStorage:', lookupPostId)
        return post
      } catch (error) {
        console.error('Error parsing local post:', error)
      }
    }
    
    // Fallback to distributed storage (for posts from other users)
    const distributedPost = await this.storage?.get(`posts.${lookupPostId}`)
    if (distributedPost) {
      console.log('✅ Found post in distributed storage:', lookupPostId)
      return distributedPost
    }
    
    console.log('❌ Post not found anywhere:', lookupPostId, 'from original ID:', postId)
    return null
  }
  ...
  async addComment(postId: string, content: string, image?: string): Promise<Post | null> {
    if (!this.currentUser) {
